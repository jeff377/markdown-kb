...
> **所屬專案：** Markdown 知識庫平台
> **技術棧：** ASP.NET Core (.NET 10)、LINE Messaging API、Microsoft Bot Framework、Swashbuckle（OpenAPI）
> **預估工期：** 3 週
> **文件版本：** 1.0
> **建立日期：** 2026-03-28
...
將 Phase 3 的 RAG 問答能力延伸至三個外部通路：
1. **LINE Bot** — 透過 LINE 聊天室查詢知識庫
2. **Microsoft Teams Bot** — 在 Teams 頻道或私訊查詢知識庫
3. **ChatGPT GPT Actions** — 讓 ChatGPT 呼叫本系統的搜尋與問答 API
...
│       └── TeamsController.cs         # POST /api/webhook/teams（新增）
│
└── MarkdownKB.Channels                ← Phase 4 新增（Class Library）
    ...
    ├── Teams/
    │   ├── TeamsActivityHandler.cs    # 處理 Bot Framework Activity
    │   └── TeamsAdaptiveCard.cs       # 格式化 Adaptive Card 回覆
...
```
Week 1：ChatGPT GPT Actions（Step 1–2）
Week 2：LINE Bot（Step 3–4）
Week 3：Teams Bot（Step 5–6）+ 整合測試（Step 7）
```

> **建議開發順序：** ChatGPT GPT Actions → LINE Bot → Teams Bot
> 原因：ChatGPT 只需 OpenAPI schema，不需新 Webhook，風險最低；LINE 次之；Teams 需要 Azure 資源，最複雜，留最後。
...
## Week 3 — Teams Bot + 整合測試

### Step 5 — 建立 Teams Bot 基礎設施（1 天）

Teams Bot 需要 Azure Bot Service 資源。

**Azure 設定步驟：**

1. 前往 [Azure Portal](https://portal.azure.com)
2. 建立「Azure Bot」資源（免費 F0 方案即可）
3. 取得：
   - `MicrosoftAppId`（Application ID）
   - `MicrosoftAppPassword`（Client Secret）
4. Messaging Endpoint 設定為：`https://{ngrok-url}/api/webhook/teams`
5. 在 Teams 中安裝 Bot（Channels → Microsoft Teams → Enable）

**加入環境變數：**

```
Teams__AppId=你的 App ID
Teams__AppPassword=你的 App Password
```

**安裝套件（MarkdownKB.Channels）：**

```bash
dotnet add src/MarkdownKB.Channels package Microsoft.Bot.Builder
dotnet add src/MarkdownKB.Channels package Microsoft.Bot.Builder.Integration.AspNet.Core
```

---

### Step 6 — 實作 Teams Bot（3 天）

#### TeamsActivityHandler.cs

繼承 `ActivityHandler`，覆寫 `OnMessageActivityAsync`：
    ...
    CancellationToken cancellationToken)
{
    ...
    var response = await ragService.ChatAsync(userId, message);

    // 格式化 Adaptive Card 回覆
    var card = TeamsAdaptiveCard.Build(response.Answer, response.Citations);
    await turnContext.SendActivityAsync(
        MessageFactory.Attachment(card), cancellationToken);
}
```

#### TeamsAdaptiveCard.cs

建立包含回答與引用來源的 Adaptive Card：

```json
{
  "type": "AdaptiveCard",
  "body": [
    { "type": "TextBlock", "text": "{回答內容}", "wrap": true },
    { "type": "TextBlock", "text": "**引用來源**", "weight": "Bolder" },
    { "type": "TextBlock", "text": "[1] attractions/yushan.md", "color": "Accent" }
  ],
  "actions": [
    { "type": "Action.OpenUrl", "title": "查看文件 [1]", "url": "{viewerUrl}" }
  ]
}
```
...
使用 Bot Framework Adapter 處理 Activity：
- 驗證 Bot Framework 簽章
- 派發給 TeamsActivityHandler
```
...
| Teams Bot | 傳送訊息 | Adaptive Card 格式正確顯示 |
| Teams Bot | 點擊引用連結 | 跳轉至 Viewer 對應頁面 |
...
| `TeamsActivityHandler` | 處理 Teams Bot Activity |
| `TeamsAdaptiveCard` | 建立 Adaptive Card 格式回覆 |
| `LineController` | POST /api/webhook/line |
| `TeamsController` | POST /api/webhook/teams |
...
─────────────────────────────────────────────
Step 5   Teams Bot 基礎設施              1.0 天
Step 6   Teams Bot 實作                  3.0 天
Step 7   整合測試                        2.0 天
─────────────────────────────────────────────
合計                                  約 11 天（3 週）
```
...
- **Teams 需要 Azure**：即使是免費方案，仍需要 Azure 帳號與訂閱
- **ngrok 免費版限制**：URL 每次重啟會改變，每次需同步更新 LINE / Teams / ChatGPT 的 Webhook 設定；建議使用固定 domain 或 ngrok 付費方案
- **Session 設計**：LINE 用 `userId`、Teams 用 `activity.From.Id` 作為 sessionId，確保每位使用者有獨立對話上下文
...
- [ ] Teams Bot 可回覆 Adaptive Card 格式的回答與引用
