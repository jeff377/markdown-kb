...
- **多通路存取**：支援 Web、LINE、Teams、ChatGPT 等多種入口
...
┌─────────────────────────────────────────────────────────┐
│                    Channel Layer                         │
│   Web UI │ LINE Bot │ Teams Bot │ ChatGPT GPT Actions   │
└──────────────────────────┬──────────────────────────────┘
                           │
┌──────────────────────────▼──────────────────────────────┐
│                    Answer Layer                          │
...
├── MarkdownKB.Channels               ← Phase 4：LINE / Teams / ChatGPT
...
| `MarkdownKB.Channels` | Class Library | Phase 4 | LINE Bot、Microsoft Teams Bot、ChatGPT GPT Actions 整合 |
...
> **v1.2 調整說明：** 根據實務評估，各 Phase 工時普遍低估 30–40%，主要原因為：路徑轉換與 Private Repo 流程未計工時（Phase 1）、Chunking 策略需要實驗時間（Phase 2）、多輪對話管理複雜度被低估（Phase 3），以及個人兼職開發本來就需要緩衝。總工期由 11 週調整為 **14–16 週**。
...
### Phase 4 — 多通路整合（預估 3 週）

**目標：** 將問答能力延伸至 LINE、Teams、ChatGPT

| 任務 | 工時估計 |
|------|---------|
| 共用問答 API 模組化 | 2 天 |
| LINE Bot（Messaging API） | 3 天 |
| Teams Bot（Bot Framework） | 4 天 |
| ChatGPT GPT Actions（OpenAPI schema） | 3 天 |
| 整合測試 | 3 天 |

**交付物：** 可透過 LINE、Teams、ChatGPT 查詢知識庫
...
## 七、如何使用 Claude Projects 管理此專案

### 建議的 Project 設置

1. **建立專案**：在 Claude.ai 左側點選「New Project」，命名為「Markdown 知識庫」

2. **上傳專案文件**（Project Instructions 或上傳檔案）：
   - 本開發計劃文件
   - API 設計文稿
   - 技術決策記錄（ADR）

3. **設定 System Prompt（Project Instructions）建議內容：**
   ```
   你是這個 Markdown 知識庫專案的開發助理。
   專案 Phase 1 使用 C# (.NET 8 + ASP.NET Core + Markdig) 技術棧。
   後續 Phase 2–4 將整合 pgvector、OpenAI SDK for .NET 等延伸功能。
   請根據上傳的開發計劃，協助我完成各 Phase 的實作任務。
   回答請以繁體中文為主，程式碼可使用英文。
   ```

4. **各 Phase 建議使用方式：**
   - Phase 1：請 Claude 幫你設計 GitHubService、MarkdownService，或產生 Razor Pages 頁面程式碼
   - Phase 2：討論 Chunking 策略、調整搜尋 relevance
   - Phase 3：設計 RAG prompt template、評估回答品質
   - Phase 4：產生 LINE Bot / Teams Bot 整合程式碼

5. **善用 Project 的記憶功能**：同一個 Project 內的對話會共享上下文，不需要每次重新解釋專案背景。

---

*文件版本：1.3 | 最後更新：2026-03-25 | 變更：新增 3.3 Solution 專案結構，說明各專案類型、職責與相依關係*
